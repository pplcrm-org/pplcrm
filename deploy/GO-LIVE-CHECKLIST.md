...
- Migrations: **CI runs them** — the `migrate` job in `deploy.yml` (added 2026-07-23 after an outage
  ...
  Manual fallback: `node --env-file=.env.production --import tsx apps/backend/src/app/migrate-cli.ts`
  from an IP allow-listed on the Postgres firewall, using the quoted owner creds. (`migrate-cli.ts`
  needs only the `DB_*` vars — it deliberately does not import `env.ts`, so it never demands the
  app's production secrets.)
...
- [x] Add HTTP **health probes** via a YAML patch — **applied 2026-07-21** (revision
      `pplcrm-api--0000029`). Container Apps ignores the Docker HEALTHCHECK and
      `az containerapp update` has no probe flags, so YAML is the only path. Liveness must stay on `GET /`
