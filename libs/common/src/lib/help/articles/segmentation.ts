      ...
      { kind: 'h2', id: 'create', text: 'Create a list' },
      {
        ...
        items: [
          ...
          },
          {
            ...
            detail:
              'Compose conditions in the query builder: match all or any, with nested groups. For people you can rule on tags and issues, on standing (volunteer status, subscriber status, staff status, support level, voting status, do not contact), on any contact or address field, on the electoral areas a household sits in — so “everyone in precinct 12” is one rule — and on activity history: days since their last donation, dollars given this year, an active recurring pledge, days since their door was last knocked, days since they last opened a newsletter, and days since their last event registration or volunteer shift. “Gave last year but not this year”, “never knocked in this campaign”, “hasn’t opened anything in 180 days” are each one or two rules; the day-count fields compare as numbers (“is at most 90” = within the last 90 days) and “is not set” means it never happened. Donations, knocks and event registrations count within the campaign context the list belongs to; newsletter opens and shifts count workspace-wide. Household lists get the knocked-days field too. The area field is labelled with your campaign’s word for it and is always offered; until your workspace has a [boundary map](/help/district-boundaries) or imported area names, it simply matches nobody. Status fields offer a picker rather than a text box, plus “is set” / “is not set”. The live preview does the math in public: “Matches 1,284 people right now”, with a note reminding you whether that count will keep moving (Smart) or freeze on save (Static).',
          },
          {
            title: 'Create it',
            detail:
