...
export const SECURITY_DOC: LegalDoc = {
  ...
  blocks: [
    ...
    {
      kind: 'p',
      text: 'Every change to a record is written to the workspace activity log with who, what and when, including actions taken through volunteer links, which are labeled as such. Exports are logged too, so an admin can always answer “who pulled the list”. Activity is retained for 90 days and exportable. When a background mailbox sync fails, the error recorded on the connection carries a support code that lets us find the exact server-side event without you sending us your data. Our servers are also monitored around the clock: automated probes running outside our hosting provider check the service that stores and serves your data every few minutes and page us when it is unreachable, background processing is checked on the same cycle, and server errors are reported to an error-tracking service (with credentials and workspace content stripped — see the privacy policy’s subprocessor list) so we usually know about a problem before you do.',
    },
    {
      kind: 'h2',
      id: 'honesty',
