...
export class DonationReceiptsController extends BaseController<'donation_receipts', ReceiptsRepo> {
  ...
  public async cancelReceipt(
    ...
    if (!receipt) throw new NotFoundError('Receipt not found');
    // A statement is a donor's whole-year summary, not a numbered document — cancelling it here
    // would void every gift it covers at once, and only a full year-end rerun brings it back.
    // The reissue path below already refuses statements for the same reason.
    if (receipt.kind === 'statement') {
      throw new BadRequestError('Statements are regenerated by rerunning the year, not cancelled one by one.');
    }
