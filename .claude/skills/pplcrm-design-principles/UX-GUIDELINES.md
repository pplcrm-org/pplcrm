...
| Job                           | The one idiom                                                                                                                                                                                                                                                                                                                                                           |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Blocking decision             | ConfirmDialogService; danger variant; **safe action styled primary** ("Keep person" / "Keep editing"); body specific enough to earn the interruption (names fields/counts). When a non-lossy third way out exists, it is a `choose()` with that option emphasized — leave prompts are **Save changes / Discard changes / Keep editing**, never just discard-or-stay     |
...
| Empty state                   | `pc-empty-state` (libs/uxcommon): icon + sentence naming cause + one projected action button. Never italic grey text or bespoke dashed containers                                                                                                                                                                                                                       |
| Form/tool dialog              | `pc-modal-shell` (libs/uxcommon): house header + `[pc-modal-footer]` slot; declarative `[open]` or imperative `show()`/`close()`. Blocking yes/no stays on ConfirmDialogService                                                                                                                                                                                         |
...
- **Body/main text is `text-xs` (12px) everywhere** — detail pages, grids, settings panels, activity logs, dialogs. This is enforced globally: the `body` rule in `styles.css` sets `font-size: 0.75rem` with the matching text-xs line-height, so unsized elements default to 12px and anything larger is an explicit opt-in — never rely on an element having no text-\* class to get bigger type. One notch up (`text-sm`, 13–14px) is reserved for the field-value emphasis in `pc-detail-item` and for section headings paired with weight (`text-sm font-bold`). Never `text-base` or arbitrary `text-[13px]` for body copy. Deliberate exception: the Help Center reads like documentation and stays larger.
- Micro-labels (eyebrows, section headers, column headers): use the `.pc-eyebrow` utility (`styles.css`) — 11px / 600 / UPPERCASE / .08em / base-content 55%, the pinned canonical of the 10–11.5px band. Don't hand-roll `uppercase tracking-*` stacks. Timestamps in activity logs: `text-[10px]`.
