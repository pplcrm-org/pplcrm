...
| Secondary action     | `btn btn-outline btn-primary`                       | Everything actionable that isn't the main action, cancel, archive, or delete. The outline **is** the demotion, so a secondary action wears the same primary hue as the main action beside it rather than introducing a second color. Never `btn-outline btn-accent` (that reads as Cancel) or `btn-outline btn-secondary` (retired 2026-07-30: the teal read as a different _kind_ of action, not a quieter one) |
